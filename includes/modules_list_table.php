...
class mpi_modules_list_table extends HD_List_Table{
    ...
    {
        global $wpdb;
        $this->table_name = $wpdb->prefix . 'mpwpi_modules';
        parent::__construct(array(
            'singular' => 'mpi_modules_field', // Singular label
            'plural' => 'mpi_modules_fields', // plural label, also this well be one of the table css class
            'ajax' => false
    ...
    public function get_columns()
    {
        return array(
            ...
            'goal' => __('Goal', "morphing-portals-integration"),
            'tags' => __('Placeholder Tags', "morphing-portals-integration"),
          //  'segmentations' => __('Segmentations', "morphing-portals-integration"),
		    'labels' => __('Labels', "morphing-portals-integration"),
        );
    }
    ...
    public function process_bulk_action()
    {
        if ('delete' === $this->current_action()) {
            foreach ($_GET[$this->_args['singular']] as $item) {
                $this->delete_this_module($item);
            }
        }

        if ('force_delete' === $this->current_action()) {
            foreach ($_GET[$this->_args['singular']] as $item) {
                $this->force_delete_this_module($item);
            }
        }
    }
        ...
        if (! empty($records)) {
            foreach ($records as $rec) {
                ...
                foreach ($columns as $column_name => $column_display_name) {
                    ...
                    $goals = $wpdb->get_row( "SELECT * FROM " . MORPHING_PORTALS_WP_INTEGRATION_DATABASE_GOALS . " WHERE id ={$rec->goal_id}" );
                    $goal_name = $rec->goal_id;
                    if(isset($goals->name)){
	                    $goal_name = $goals->name;
                    }

                    // edit link
                    $editlink = menu_page_url("morphing-portals-integration-add-module", 0) . '&module_id=' . (int) $rec->id;
                    ...
                    switch ($column_name) {
                        ...
                        case "url":
                            ...
                            break;
                        case "goal":
                            echo '<td ' . $attributes . '>' . stripslashes($goal_name) . '</td>';
                            break;
                        case "tags":
                            echo '<td ' . $attributes . '>' . stripslashes($rec->tags) . '</td>';
                            break;
                   /*     case "segmentations":
                            echo '<td ' . $attributes . '>' . stripslashes($rec->exclude_segmentations) . '</td>';
                            break; */
						case "labels":
                            echo '<td ' . $attributes . '>' . stripslashes($rec->labels) . '</td>';
                            break;
                        case "cb":
    ...
    public function delete_this_module($id){
	    ...
	    $result = $comunicator->deleteEntry('bannerID');
    ...
    public function force_delete_this_module($id){
	    global $wpdb;
        /* -- Preparing the delete query to avoid SQL injection -- */
        $query = $wpdb->prepare("DELETE FROM $this->table_name WHERE id = %d", $id);
        $wpdb->query($query);
